...
class TestBasketView(TestCase):
  def setUp(self):
    ...
    Product.objects.create(category_id=1, title='Grokking Algorithm', created_by_id=1, 
                            slug='grokking-algorithm', price='34.99', image='Grokking')
    Product.objects.create(category_id=1, title='Rich Dad Poor Dad', created_by_id=1, 
                            slug='grokking-algorithm', price='34.99', image='Grokking')
    Product.objects.create(category_id=1, title='Test Book', created_by_id=1, 
                            slug='grokking-algorithm', price='34.99', image='Grokking')
    self.client.post(
  ...
  def test_basket_delete(self):
    ...
    self.assertEqual(response.json(), {'qty': 1, 'subtotal': '34.99'})

  def test_basket_update(self):
    response = self.client.post(
      reverse('basket:basket_update'), {"productid": 2, "productqty": 1, "action": "post"}, xhr=True)
    self.assertEqual(response.json(), {'qty': 2, 'subtotal': '69.98'})
